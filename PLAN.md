...
| Incomplete | Work | 5G Stage C: deterministic reclamation/budget + atomic CP pressure + degraded deny recovery/telemetry | Remaining: reclaim expired entries first and then deterministically LRU-evict only DNS-derived allows within a rolling per-attachment admission/eviction budget; preserve the admitted working set and return SERVFAIL once exhausted; atomically preflight authoritative control-plane replacements; enter and recover from visible fail-closed degradation when a deny cannot be installed; and export per-map occupancy/high-water, eviction, admission-failure, throttle, and degraded-state telemetry. Never evict control-plane/system or deny state. |
| Incomplete | Test | Capacity-and-churn traffic tests and BPF/DNS benchmarks | Stages A–B now cover exact-tier migration, ownership/refcount removal, bounded metadata/caps, working-set-preserving SERVFAIL, protected LPM overlap, DENYLIST isolation, policy-mode flips, restore ambiguity, and canonical domain aliases. Real tiny-capacity cgroup + TC matrices visibly pass. Final Stage B Docker medians remain inside the query-path gate (proxy cold/warm +16.5%/+14.4%; allowlist cold/warm -19.7%/-19.3%), with zero-allocation warm refresh and no-op expiry diagnostics. Still missing deterministic expired-first/LRU selection, rolling-budget throttling/recovery, protected-entry eviction proofs, deny-pressure degradation/recovery, and their telemetry/churn evidence. |
| Incomplete | Gate | Bootstrap/TCP/upstream/capacity tests green in Docker gate | 5A–5F and 5G Stages A–B are accepted by separate skeptical reviews. On the frozen Stage B tree, `make check-docker`, `make test-docker`, `make test-docker-cgroup`, `make test-docker-tc`, and `make bench-docker` all passed; `TestCgroupDNSExactOwnershipAndCapacityTraffic` and `TestTCDNSExactOwnershipAndCapacityTraffic` visibly ran and passed. Full Phase 5 remains gated on Stage C reclamation/budget, authoritative pressure handling, degraded recovery, telemetry, and recovery traffic above. |

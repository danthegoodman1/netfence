...
| Incomplete | Work | 5G: source-aware bounded DNS admission/eviction + atomic CP admission + fail-closed deny pressure | The exact-tier foundation is complete, but the production resolver still uses `AllowIP`/LPM. Remaining: cut resolver admission over to the exact tier; add domain→IP ownership and shared-owner refcounts; enforce per-response, per-matched-domain, per-attachment entry, TTL-metadata, domain, and ownership-edge caps; reclaim expired entries before deterministic LRU eviction within a rolling per-attachment admission/eviction budget; preserve the admitted working set and return SERVFAIL after budget exhaustion; atomically preflight authoritative control-plane replacements; enter and recover from a visible fail-closed degraded state when deny admission fails; and export/rate-limit/document occupancy high-water, eviction, admission-failure, throttle, and degraded-state telemetry through logs, heartbeat stats, README, and proto contracts. |
| Incomplete | Test | Capacity-and-churn traffic tests and BPF/DNS benchmarks | Stage A has cgroup + TC traffic matrices for exact-only admission, protected LPM precedence/removal, DENYLIST isolation, and unchanged DISABLED/BLOCK_ALL modes; real tiny-capacity maps prove atomic two-family preflight and unchanged occupancy on rejection; genuine legacy migration and crash/ambiguity tests prove enforcement continuity and retry convergence. `make bench-docker` five-sample medians measured an exact hit at ~3.005 us (+1.7% vs baseline, +0.9% vs protected LPM), with DNS benchmarks within the project’s ±20% threshold. Still missing resolver-level ownership/refcount removal, bounded metadata/cap tests, expired-first/LRU selection, rolling-budget throttling, working-set preservation/SERVFAIL behavior, deny-pressure degradation/recovery, and expiry/window-recovery traffic. |
| Incomplete | Gate | Bootstrap/TCP/upstream/capacity tests green in Docker gate | 5A–5E and the 5G Stage A exact-tier foundation are accepted by skeptical review. Independent `make check-docker`, `make test-docker`, `make test-docker-cgroup`, `make test-docker-tc`, and `make bench-docker` gates are green; exact-tier cgroup + TC traffic, genuine pinned-program migration, and tiny-capacity tests explicitly ran and passed. Full Phase 5 remains gated on the 5F/remaining-5G resolver ownership, bounded-capacity/LRU-budget, pressure, telemetry, and degraded-recovery evidence above. |
